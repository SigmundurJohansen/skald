...
void generate_makefile(make_settings &settings, project &project) {
  ...
  if (makefile.is_open()) {
    ...
    for (int i = 1; i < project.c_files.size(); i++) {
    ...
    for (int i = 0; i < project.cpp_files.size(); i++) {
    ...
    for (int i = 0; i < project.folders.size(); i++) {
    makefile << "\t@if not exist \"$(BUILDDIR)\\" << project.folders[i] << "\" mkdir \"$(BUILDDIR)\\" << project.folders[i] << "\"\n";
    }
